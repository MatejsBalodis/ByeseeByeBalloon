...
var mouse_position_on_button_press = Vector2() # To detect whether there was a drag.
...
const MAX_ACCEPT_DISTANCE = 64.0 # If the item selection bar has moved more than this, don't accept item.
...
func _on_UpItem_button_down():
	if !item_is_pressed:
		mouse_position_on_button_press = get_viewport().get_mouse_position()
		item_is_pressed = true

func _on_UpItem_button_up():
	if !must_fall:
		item_is_pressed = false
		if mouse_position_on_button_press.distance_squared_to(get_viewport().get_mouse_position()) < MAX_ACCEPT_DISTANCE && !player.item_drop_pending:
