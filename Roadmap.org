...
* TODO When there are no items in the bar the white arrow is still displayed, when the bar is dragged.
* TODO Add type hints to all variables.
...
* TODO There is still a problem that dropped items appear behind the background now.
** Can I set render queue or some z index somehow?
* TODO Also have to implement so that if mouse moves over the bottom bar the coefficient increases the strength of the effect.
