...
* TODO Fix so that if items are dropped and bar has become less wide, it isn't possible to drag it too far to the left, when bar is wider that viewport width.
* IN-PROGRESS Level value points must be tuned so that the growth of the total score is taken into account.
** I added the whole previous level value to each next, so that if player plays well he makes his life easier in long term.
*** Though it's not balanced and Matejs should balance out longterm and shortterm spendings.
